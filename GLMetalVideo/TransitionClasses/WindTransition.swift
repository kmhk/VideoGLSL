...
final class WindTransition : TransitionRenderer {
    
    var wind_size = 0.2
    
    init(asset: AVAsset, asset1: AVAsset) {
        super.init(asset: asset, asset1: asset1, function: "transition_wind")
    }
    ...
    public override func render(pixelBuffer: CVPixelBuffer, pixelBuffer2: CVPixelBuffer, progress: Float) -> MTLTexture? {
        ...
        var wind_size = Float(self.wind_size)
        computeCommandEncoder!.setBytes(&wind_size, length: MemoryLayout<Float>.size, index: 1)
