...
final class AngularTransition : TransitionRenderer {
    
    var startingAngle = 90
    
    init(asset: AVAsset, asset1: AVAsset) {
        super.init(asset: asset, asset1: asset1, function: "transition_angular")
    }
    ...
    public override func render(pixelBuffer: CVPixelBuffer, pixelBuffer2: CVPixelBuffer, progress: Float) -> MTLTexture? {
        ...
        var startingAngle = Int(self.startingAngle)
        computeCommandEncoder!.setBytes(&startingAngle, length: MemoryLayout<Int>.size, index: 1)
