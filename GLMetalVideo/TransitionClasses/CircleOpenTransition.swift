...
final class CircleOpenTransition : TransitionRenderer {
    
    var smooth = 0.3
    var open = true
    
    init(asset: AVAsset, asset1: AVAsset) {
        super.init(asset: asset, asset1: asset1, function: "transition_circle_open")
    ...
    public override func render(pixelBuffer: CVPixelBuffer, pixelBuffer2: CVPixelBuffer, progress: Float) -> MTLTexture? {
        ...
        var var_1 = Float(self.smooth)
        computeCommandEncoder!.setBytes(&var_1, length: MemoryLayout<Float>.size, index: 1)
        var var_2 = Bool(self.open)
        computeCommandEncoder!.setBytes(&var_2, length: MemoryLayout<Bool>.size, index: 2)
