...
final class LinearBlurTransition : TransitionRenderer {
    
    var intensity = 0.1
    var passes = 6
    
    init(asset: AVAsset, asset1: AVAsset) {
        super.init(asset: asset, asset1: asset1, function: "transition_linearblur")
    }
    ...
    public override func render(pixelBuffer: CVPixelBuffer, pixelBuffer2: CVPixelBuffer, progress: Float) -> MTLTexture? {
        ...
        var intensity = Float(self.intensity)
        computeCommandEncoder!.setBytes(&intensity, length: MemoryLayout<Float>.size, index: 1)
        var passes = Int(self.passes)
        computeCommandEncoder!.setBytes(&passes, length: MemoryLayout<Int>.size, index: 2)
