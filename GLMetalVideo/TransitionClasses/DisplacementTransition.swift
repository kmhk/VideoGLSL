...
final class DisplacementTransition : TransitionRenderer {
    
    var strength = 0.05
    
    init(asset: AVAsset, asset1: AVAsset) {
        super.init(asset: asset, asset1: asset1, function: "transition_displacement")
    }
    ...
    public override func render(pixelBuffer: CVPixelBuffer, pixelBuffer2: CVPixelBuffer, progress: Float) -> MTLTexture? {
        ...
        var strength = Float(self.strength)
        computeCommandEncoder!.setBytes(&strength, length: MemoryLayout<Float>.size, index: 1)
