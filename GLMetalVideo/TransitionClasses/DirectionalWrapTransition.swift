//
//  DirectionalWrapTransition.swift
//  VideoTrasition
//
...
final class DirectionalWrapTranstion : TransitionRenderer {
    ...
    init(asset: AVAsset, asset1: AVAsset) {
        super.init(asset: asset, asset1: asset1, function: "transition_directional_wrap")
    }
    ...
    public override func render(pixelBuffer: CVPixelBuffer, pixelBuffer2: CVPixelBuffer, progress: Float) -> MTLTexture? {
        ...
        var cx = Float(self.center.x)
        computeCommandEncoder!.setBytes(&cx, length: MemoryLayout<Float>.size, index: 4)
        var cy = Float(self.center.y)
        computeCommandEncoder!.setBytes(&cy, length: MemoryLayout<Float>.size, index: 5)
